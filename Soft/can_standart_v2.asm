...
; *                                                                 *
; * This code implements SPI communications for the PIC12F675       *
; * using a three wire interface and a MCP2515 CAN controller.      *
; *******************************************************************

; *******************************************************************
; * Description release:                                            *
; *                                                                 *
; * when WDT event - wake up from WDT (18ms) and read pin TXRTS,    *
; *                  choice and toggle pin RX1BF MCP2515,           *
; *                  every 90ms: transmit value ADC0.               *
; *                                                                 *
; * when RTR msg   - transmit value ADC1.                           *
; *******************************************************************
...
; GPIE set - interrupt on pin change
        ...
        movlw	0x00				; all interrupts disabled
        movwf	INTCON				; -
		clrwdt

; *******************************************************************
; * Main cycle - sleep, wake up from WDT		                    *
; *******************************************************************
main
        sleep						; SLEEP
        nop                         ;
        call	service_wdt         ; service wdt
        call	service_int			; service gpio
...
; * TXB0 - 3FD - Analog Value Channel 0,(every one 90ms - from Timer0 ) *
...
; Mode NORMAL
        ...
        call    spi_send                
        movlw   REQOP_NORMAL|OSM_ENABLE 
        call    spi_send			; Normal and One Shot Mode
...
; *******************************************************************
service_wdt
; Counter for one second = 5 * 18ms = 90 ms
		incf	delay_cnt, F
		movlw	.5
		xorwf	delay_cnt, W
...
;--------------------------------------------------------------------
; Read input and write output MCP2515 every 18ms
pin_mcp2515
        bcf     GPIO,cs_pin         ; enable MCP2515
...
; * Service GPIO  			                                        * 
; *******************************************************************
service_int                         ; Message received interrupt
		btfsc   GPIO, int_pin       ; if ~INT = 0 (MCP2515 interrupt)
		return						; no, return
									; yes, reading CANINTF
		movlw    CAN_READ                
